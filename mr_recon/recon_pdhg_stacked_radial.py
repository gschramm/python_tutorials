...
from operators import MultiChannel3DNonCartesianStackedMRAcquisitionModel, ComplexGradientOperator
...
sens = np.ones((num_channels, n2, n, n, 2)) / 20000
sens[..., 1] = 0

# setup the data operator for a 3D radial sequence
phis = 2 * np.pi * np.random.rand(num_spokes)
kspace_sample_points = np.zeros((num_spokes * num_samples_per_spoke, 2))
stack_numbers = np.zeros(num_spokes * num_samples_per_spoke, dtype=np.int16)
...
for isp in np.arange(num_spokes):
    ...
    kspace_sample_points[offset:(offset + num_samples_per_spoke),
                         1] = r * np.sin(phis[isp])
    stack_numbers[offset:(offset + num_samples_per_spoke)] = np.random.randint(
        0, n2)

# choose random stack numbers

data_operator = MultiChannel3DNonCartesianStackedMRAcquisitionModel(
    (n2, n, n), num_channels, sens, kspace_sample_points, stack_numbers)
...
for ch in range(num_channels):
    for i in range(2):
        noisy_data_sampling_corrected[ch, :,
                                      i] *= np.tile(np.abs(r), num_spokes)
...
ax[2, 0].imshow(np.linalg.norm(reconstructor.x, axis=-1)[n2 // 2, ...], **ims)
ax[2, 1].imshow(reconstructor.x[n2 // 2, ..., 0], **ims)
ax[2, 2].imshow(reconstructor.x[n2 // 2, ..., 1], **ims)
